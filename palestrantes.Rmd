...
```{r palestrantes, eval = file.exists("2024/Palestrantes.xlsx"), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', out.width = "100%", results = "asis"}
library("dplyr")

# initialize all sheets
abas <- readxl::excel_sheets("2024/Palestrantes.xlsx")

# display each sheet in a tab
for (j in 1:length(abas)) {
  palestrantes.raw <-
    readxl::read_excel("2024/Palestrantes.xlsx",
                       sheet = abas[j],
                       col_types = c("text"))
  ...
  for (i in 1:dim(palestrantes.raw)[1]) {
    if (!is.na(palestrantes.raw[i, 1])) {
      fotos[i] <-
        paste0('<img src=\"',
               file.path("2024", "Palestrantes", palestrantes.raw[i, 1]),
